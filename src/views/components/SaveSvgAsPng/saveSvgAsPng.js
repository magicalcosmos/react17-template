...
const saveSvgAsPng = (function() {
  ...
  var styleSheetRules = function styleSheetRules() {
    ...
    cachedRules = Array.from(document.styleSheets).map(function(sheet) {
      try {
  ...
  out$.prepareSvg = function(el, options, done) {
    ...
    const excludeClassName = _ref3.excludeClassName;
    const excludeClassAttribute = _ref3.excludeClassAttribute;
    ...
    return inlineImages(el).then(function() {
      ...
      if (el.tagName !== 'svg') {
        if (el.getBBox) {
          if (clone.getAttribute('transform') != null) {
            ...
            clone.setAttribute('transform', clone.getAttribute('transform').replace(/scale\(.*?\)/, ''));
          }
      ...
      //删除不需要的类
      if (excludeClassName && excludeClassName.length > 0) {
        excludeClassName.forEach((className) => {
          clone.querySelectorAll(`.${className}`).forEach(item => item.remove());
        }); 
      }
      //删除样式中的class属性
      if (excludeClassAttribute && excludeClassAttribute.length > 0) {
        excludeClassAttribute.forEach((className) => {
          clone.querySelectorAll(`.${className}`).forEach(item => item.removeAttribute('class'));
        });
      }
  ...
  out$.svgAsPngUri = function(el, options, done) {
    ...
    var _ref5 = options || {},
      ...
      encoderType = _ref5$encoderType === undefined ? 'image/jpeg' : _ref5$encoderType,
    ...
    var convertToPng = function convertToPng(_ref6) {
      ...
      var pixelRatio = (window.devicePixelRatio || 1) * scale;

      canvas.width = width * pixelRatio;
      canvas.height = height * pixelRatio;
      canvas.style.width = canvas.width + 'px';
      canvas.style.height = canvas.height + 'px';
      //canvas设置背景颜色，不然生成的图片背景色是黑的
      ...
      try {
        ...
        if (typeof SecurityError !== 'undefined' || e.name === 'SecurityError') {
  ...
  return {
    saveSvgAsPng: out$.saveSvgAsPng,
    saveSvg: out$.saveSvg,
    download: out$.download,
    svgAsPngUri: out$.svgAsPngUri,
    svgAsDataUri: out$.svgAsDataUri,
    prepareSvg: out$.prepareSvg
  };
})();
